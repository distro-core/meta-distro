...
A corrupted WSL instance may not start; in this case, unregister the WSL instance and then reinstall. It may
be necessary to restart Windows before WSL will respond and allow the unregister command to function.

```batch
wsl.exe --unregister ubuntu-22.04
mkdir "%LOCALAPPDATA%\WSL\ubuntu-22.04"
wsl.exe --import ubuntu-22.04 "%LOCALAPPDATA%\WSL\ubuntu-22.04" "%USERPROFILE$\Downloads\wsl-ubuntu-22.04.tar.gz"
wsl.exe --distribution ubuntu-22.04 --cd ~
```

```bash
/usr/local/bin/setup-home-links.sh
```
