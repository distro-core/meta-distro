...
for distro in ${distros[*]} ; do
    ...
    for machine in ${machines[*]} ; do
        ...
        if [[ -n "$clean_sstate" ]]; then
            clean_dir "build/sstate-cache/$machine/*"
        else
            echo "NOTE: retain build/sstate-cache/$machine"
        fi
...
for distro in ${distros[*]} ; do
    ...
    for machine in ${machines[*]} ; do
        ...
        bitbake_rc=0
        bitbake ${targets_flags[*]} ${targets[*]} || bitbake_rc=$?
        echo bitbake_rc=$bitbake_rc

        if [[ $bitbake_rc -eq 0 ]] ; then
            distro_codename=$(bitbake-getvar --quiet --value DISTRO_CODENAME)
            repo manifest --pretty --output-file distro-head-$distro_codename.xml
            repo manifest --pretty --revision-as-HEAD --output-file distro-refs-$distro_codename.xml
        fi
