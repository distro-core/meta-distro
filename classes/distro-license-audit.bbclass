...
def local_license_audit(pkgs, licenses, d):
    ...
    _ = d  # Indicate that d is intentionally unused
    pkg_list = pkgs.split()
    lic_list = licenses.split()
    base_packages = ["", "-dbg", "-dev", "-doc", "-help", "-locale", "-ptest", "-staticdev", "-src", "-tool"]
    retval_list = []
    for pkg in pkg_list:
        for lic in lic_list:
            for base_pkg in base_packages:
                retval_list.append(f"{pkg}{base_pkg}:{lic}")
...
${@local_license_audit('pcsc-lite pcsc-lite-spy', 'GPL-3.0-or-later', d)} \
