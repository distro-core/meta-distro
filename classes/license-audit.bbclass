...
INCOMPATIBLE_LICENSE_EXCEPTIONS ?= "${LOCAL_LICENSE_AUDIT}"
...
def local_license_audit(pkgs, licenses, d):
    ...
    for pkg in pkgs.split():
        for lic in licenses.split():
            ...
            retval += f"{pkg}-locale:{lic} {pkg}-ptest:{lic} {pkg}-src:{lic} {pkg}-staticdev:{lic} "
    return retval

# *DO NOT DELETE ENTRIES* IN LOCAL_LICENSE_AUDIT; THE LIST IS UTILIZED TO BY EXCLUSION
# TO TERMINATE BITBAKE UNTIL THE LICNESE IS ADDED FOR EXCEPTION PROCESSES.
