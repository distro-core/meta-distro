...
rootfs_postuninstall_usr_share_factory() {
    ...
    if [ -d ${IMAGE_ROOTFS}/efi ]; then
        ...
        cat <<EOF >${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/00-boot-${BPN}-00.conf
...
EOF
    ...
    if [ -d ${IMAGE_ROOTFS}/boot ]; then
        ...
        # https://www.freedesktop.org/software/systemd/man/latest/tmpfiles.d.html
        cat <<EOF >${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/00-boot-${BPN}-01.conf
...
EOF
    ...
    # https://www.freedesktop.org/software/systemd/man/latest/tmpfiles.d.html
    cat <<EOF >${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/00-boot-${BPN}-02.conf
