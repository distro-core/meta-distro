...
inherit image_types_ostree_extend
inherit image_types_ota_extend
...
ROOTFS_PREPROCESS_COMMAND:prepend = "${@bb.utils.contains_any('DISTRO_FEATURES','usrmerge sota','rootfs_preprocess_usr_share_factory;','',d)}"
...
# }
# ROOTFS_POSTINSTALL_COMMAND:append = "${@bb.utils.contains_any('DISTRO_FEATURES','usrmerge sota','rootfs_postinstall_usr_share_factory;','',d)}"
...
# }
# ROOTFS_POSTPROCESS_COMMAND:append = "${@bb.utils.contains_any('DISTRO_FEATURES','usrmerge sota','rootfs_postprocess_usr_share_factory;','',d)}"
...
rootfs_postuninstall_usr_share_factory() {
    ...
    if [ -d ${IMAGE_ROOTFS}/efi ]; then
        ...
        cat >${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/00-boot-${BPN}-00.conf <<EOF
...
EOF
    ...
    if [ -d ${IMAGE_ROOTFS}/boot ]; then
        ...
        # https://www.freedesktop.org/software/systemd/man/latest/tmpfiles.d.html
        cat >${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/00-boot-${BPN}-01.conf <<EOF
...
EOF
    ...
    # https://www.freedesktop.org/software/systemd/man/latest/tmpfiles.d.html
    cat >${IMAGE_ROOTFS}${nonarch_libdir}/tmpfiles.d/00-boot-${BPN}-02.conf <<EOF
...
ROOTFS_POSTUNINSTALL_COMMAND:append = " ${@bb.utils.contains_any('DISTRO_FEATURES','usrmerge sota','rootfs_postuninstall_usr_share_factory;','',d)}"
...
# }
# IMAGE_POSTPROCESS_COMMAND:append = " ${@bb.utils.contains_any('DISTRO_FEATURES','usrmerge sota','image_postprocess_usr_share_factory;','',d)}"
